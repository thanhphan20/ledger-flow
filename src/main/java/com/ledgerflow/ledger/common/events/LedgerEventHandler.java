...
public class LedgerEventHandler {
    ...
    public void handlePaymentCompleted(PaymentCompletedEvent event) {
        System.out.println("Ledger: Payment completed, creating user account if not exists: " + event.getPaymentId());
        ledgerService.createEntry(event.getPaymentId(), event.getAmount(), event.getCurrency(), EntryType.CREDIT);
    }
}
